...
from cookiebreaks.api.routers.utils import BreakExternal as Break, break_internal_to_external
...
async def claim_break(
    ...
    external_breaks =  list(map(lambda c: break_internal_to_external(c, current_user), updated_breaks))
    external_claims =  list(map(lambda c: claim_internal_to_external(c, current_user), updated_claims))
